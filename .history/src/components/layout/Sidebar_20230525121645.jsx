          ...
          {nav.map((nav) => {
            return (
              <Link to={nav.link} key={nav.name} >
                <span
                  className={nav.link === location.pathname ? 'active-menu flex my-3 items-center cursor-pointer   tracking-wider p-2 rounded-sm ' :"flex my-3 items-center cursor-pointer   tracking-wider p-2 rounded-sm " }
                  style={{ color: "#aac0bb" }}
                >
                  {nav.icon} {nav.name}
                </span>
