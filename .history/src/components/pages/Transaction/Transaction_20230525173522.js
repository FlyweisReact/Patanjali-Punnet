  ...
  return (
    <>
      <section className="component-dashboard">
        ...
        {data?.length === 0 || !data ? (
          <Alert>No Transaction's Found</Alert>
        ) : (
              ...
              <tbody>
                {data?.map((i, index) => (
                  <tr key={index}>
                    ...
                    <td> <i className="fa-solid fa-indian-rupee-sign"></i> {i.amount} </td>
                    <td> {i.paymentMode === 'COD' ? <Badge bg='success'>Cash on Delivery</Badge> : ''} 
                    {i.paymentMode === 'Online' ? <Badge bg='warning'>Online</Badge> : ''} 
                    </td>
                    <td> 
