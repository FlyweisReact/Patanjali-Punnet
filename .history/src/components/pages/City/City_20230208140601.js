...
import { AiFillDelete, AiFillEdit, AiOutlineNodeIndex } from "react-icons/ai";
  ...
  const deleteHandler = async (id) => {
    try {
      const {data} = await axios.delete(
        `http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5005/api/v1/admin/product/${id}` , Auth
      );
    ...
    <>
      ...
      >
        ...
        <Table striped bordered hover>
          ...
          <tbody>
            {data?.products?.map((i, id) => (
              <tr key={id}>
                <td>
                {i.images?.map((pic ,indexid) => (
                  <img
                    src={pic}
                    key={AiOutlineNodeIndex}
                    className="fast-food"
                    alt="ProductImage"
                  />
                ))}
