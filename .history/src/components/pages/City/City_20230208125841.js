...
const City = () => {
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const AddProduct = async (e) => {
      ...
      try{
          const data = await axios.post('http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5005/api/v1/admin/product/new' , fd , Auth)
          console.log(data)
      }catch(err){
        console.log(err)
      }
          ...
          <Container>
            ...
            <Form >
              <Form.Group className='mb-3'>
                ...
                {image.length > 0
                ? image.map((pic, index) => <div key={index} style={{marginBottom: '10px'}}>{pic.name}</div>)
                : null}
                <Form.Control
                  type="file"
                  accept="image/*"
