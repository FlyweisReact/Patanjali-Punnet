...
const City = () => {
  ...
  const fetchHandler = async () => {
    try {
      const { data } = await axios.get(
        "http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5005/api/v1/admin/products" , Auth
      );
      setData(data);
    } catch (err) {
      console.log(err);
  ...
  const deleteHandler = async (id) => {
    try{
      ...
      const data = await axios.delete(`http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5002/cityRouter/deletepost/${id}`)
        ...
        <Table striped bordered hover>
          ...
          <tbody>
            {data?.products?.map((i, index) => (
              <tr key={index}>
                <td> 
                <img src={i.images[0]?.url} alt='Product Image' className="fast-food" />
                 </td>
                <td> {i.name} </td>
                <td> {i.description} </td>
                <td> {i.price} </td>
