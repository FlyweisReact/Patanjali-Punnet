...
const City = () => {
  ...
  const getPatanjali = async () => {
    try {
      const { data } = await axios.get(
        "https://puneet-goyal-backend.vercel.app/api/v1/getAllCategory" , Auth
      );
  ...
  const getNonPatanjali = async () => {
    try {
      const { data } = await axios.get(
        "https://puneet-goyal-backend.vercel.app/api/v1/non-pantangli/getAllNonPanangli" , Auth
      );
      setNP(data.message);
    } catch (err) {
      console.log(err);
        ...
        <Modal.Body>
          <Container>
            <Form onSubmit={AddProduct}>
              ...
              <Button variant="outline-submit" type="submit">
                Submit
              </Button>
            </Form>
          </Container>
