...
const NotifyCustomer = () => {
  ...
  const fetchhandler = async () => {
    try {
      const { data } = await axios.get(
        "http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5005/api/v1/coupon/all" , Auth
      );
  ...
  const deleteHandler = async (id) => {
    try {
      const {data} = await axios.delete(
        `http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5005/api/v1/coupon/${id}` , Auth
      );
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const postHandler = async (e) => {
      e.preventDefault()
      const FormD =  {  couponCode , minOrder , expirationDate , activationDate , discount }
      try{
          const { data } = await axios.post('http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5005/api/v1/coupon' , FormD , Auth )
          toast.success(data?.msg)
          fetchhandler()
          setModalShow(false)
      }catch(err){
          ...
          <Container>
            <Form onSubmit={postHandler}>
              ...
              <Form.Group className="mb-3">
                ...
                <Form.Control type="number" min={0} onChange={(e) => setM(e.target.value)}  />
              </Form.Group>
              ...
              <Form.Group className="mb-3">
                ...
                <Form.Control type="number" min={1} onChange={(e) => setDiscount(e.target.value)}  />
              </Form.Group>
